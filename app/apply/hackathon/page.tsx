...
export default function HackathonApplyPage() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push(`/auth/login?next=/apply/hackathon`);
    }
  }, [user, loading, router]);
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="w-12 h-12 border-4 border-white border-t-gray-400 rounded-full animate-spin mx-auto" />
          <p>Yükleniyor...</p>
        </div>
      </div>
    );
    ...
    <main className="min-h-screen bg-black text-white py-20 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-12 text-center space-y-4">
          <h1 className="text-5xl font-bold text-white">HackathOn Aydin'a Başvur</h1>
          <p className="text-gray-400">
            48 saat boyunca teknoloji ile gerçek sorunlara çözüm bulma yarışmasına katılmak için
            başvur.
          </p>
        </div>
        ...
        <form className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-white"
          ...
          <div className="space-y-6 p-8 rounded-lg bg-white/5 border border-white/10">
            <h2 className="text-xl font-bold text-white">HackathOn Detayları</h2>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Takım Durumunuz *
              </label>
              <select className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-white">
            ...
            <div>
              ...
              <div className="space-y-2">
                {["Backend", "Frontend", "DevOps", "Tasarımcı", "Ürün Yöneticisi"].map((role) => (
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Teknoloji Stack'i (Virgülle ayırınız)
              </label>
              <input
                type="text"
                className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-white"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Portfolio/GitHub Linki (İsteğe bağlı)
              </label>
              <input
                type="url"
                className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-white"
          ...
          <div className="space-y-4 p-8 rounded-lg bg-white/5 border border-white/10">
            <h2 className="text-xl font-bold text-white">Onaylar</h2>

            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" required className="w-4 h-4 accent-white mt-1 rounded" />
              <span className="text-sm text-gray-300">
                <strong>KVKK Aydınlatma Metni</strong>'ni okudum ve kabul ettim. (
                <Link href="/kvkk" className="text-white hover:text-gray-300">
                  Metni Oku
                </Link>
                )
              </span>
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              className="flex-1 px-8 py-3 bg-white text-black font-bold rounded-lg hover:bg-gray-100 transition-all hover:scale-105 active:scale-95"
            >
              Başvuru Yap
            </button>
            <Link
